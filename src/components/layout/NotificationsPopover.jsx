  ...
  return (
    <div ref={rootRef} className="relative">
      <IconButton
        ...
        dot={overdue > 0}
        aria-label={`${t("notifications.title")}${overdue ? ` (${t("notifications.overdue", { n: overdue })})` : ""}`}
      >
        <Bell size={16} />
      </IconButton>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute end-0 top-[52px] z-40 w-[380px] rounded-3xl bg-[var(--surface)] border border-[var(--border)] shadow-hover overflow-hidden"
            role="dialog"
            aria-label={t("notifications.title")}
          >
            <div className="flex items-center justify-between px-5 h-12 border-b border-[var(--border)]">
              <div className="text-sm font-semibold text-[var(--ink)]">{t("notifications.recent")}</div>
              {overdue > 0 && (
                <span className="text-[11px] font-semibold text-[var(--danger)] tabular-nums">
                  {t("notifications.overdue", { n: overdue })}
                </span>
              )}
            </div>
            ...
            <div className="max-h-[420px] overflow-y-auto">
              {invoices.length === 0 ? (
                <div className="px-5 py-10 text-center">
                  ...
                  <div className="text-sm font-medium text-[var(--ink)]">{t("notifications.emptyTitle")}</div>
