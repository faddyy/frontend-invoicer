...
const NAV_DEFS = [
  { id: "nav:dashboard", kind: "nav", labelKey: "nav.dashboard", hintKey: "palette.hintDashboard", to: "/dashboard", icon: LayoutGrid },
  { id: "nav:invoices", kind: "nav", labelKey: "nav.invoices", hintKey: "palette.hintInvoices", to: "/invoices", icon: FileText },
  { id: "nav:new", kind: "nav", labelKey: "palette.createInvoice", hintKey: "palette.hintNew", to: "/invoices/new", icon: Plus },
  { id: "nav:clients", kind: "nav", labelKey: "nav.clients", hintKey: "palette.hintClients", to: "/clients", icon: Users },
  { id: "nav:settings", kind: "nav", labelKey: "nav.settings", hintKey: "palette.hintSettings", to: "/settings", icon: SettingsIcon },
];
...
export function CommandPalette({ open, onClose }) {
  ...
  const items = useMemo(() => {
    ...
    const invoiceItems = (invoices || []).map((i) => ({
      ...
      hint: `${t(`status.${i.effective_status}`)} · ${formatMoney(i.total, i.currency)}`,
      to: `/invoices/${i.id}`,
      icon: FileText,
    }));
    const clientItems = (clients || []).map((c) => ({
      ...
      kind: "client",
      label: c.name,
      hint: c.company || c.email || t("palette.clientFallback"),
      to: `/clients/${c.id}`,
      icon: Users,
    }));
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 h-10 border-t border-[var(--border)] bg-[var(--surface-2)]/60 text-[11px] text-[var(--ink-muted)]">
              ...
              <span>{items.length === 1 ? t("palette.results", { n: items.length }) : t("palette.resultsPlural", { n: items.length })}</span>
            </div>
          </motion.div>
        </motion.div>
      )}
